<template>
    <div class="ele-body">
        <a-card :bordered="false">
            <vxe-grid ref='newsCategoryGrid' v-bind="gridOptions" class="my-grid66">
                <!--搜索表单 -->
                <template #form>
                    <a-form
                            :model="where"
                        ...
                        <a-row>
                                        ...
                                        <a-col  :lg="10" :md="12" :sm="26"
                                                             :xs="24">
                                            <a-form-item label="描述:" name="status">
                                            <a-input
                                                     v-model:value="where.status"
                                                    :maxlength="255"
                                                     placeholder="请输入关于新闻的描述"
                                                    allow-clear
                                                    class="ele-fluid"
                                            />
                                             </a-form-item>
            ...
            <!--编辑弹窗-->
            <vxe-modal v-model="newsCategoryListApp.editModalShowing" ref="newsCategoryEditModal"
                       :title="gridOptions.showEditTitle"
                       id="newsCategoryEditModal"
            ...
            >
                <template #default>
                    <div id="newsCategoryEditModalContent">
                    </div>
                </template>
                <template #footer>
                    <div id="newsCategoryEditModalFooter">
                    </div>
                    <teleport to="#newsCategoryEditModalContent">
                        <newsCategory-edit ref="newsCategory-edit"/>
                    </teleport>
                </template>
            </vxe-modal>

            <!--查看弹窗-->
            <vxe-modal v-model="newsCategoryListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                       id="newsCategoryViewModal"
                       width="600"
                       min-width="460"
                       min-height="320"
                       :destroy-on-close="true"
                       ...
                       show-footer show-zoom resize remember storage transfer dblclickZoom>
                <template #default>
                    <newsCategory-detail ref="newsCategory-detail"/>
                </template>
            </vxe-modal>
        </a-card>
    </div>
...
<script>
    ...
    import NewsCategoryEdit
        from "@/views/cms/newsCategory/newsCategory/newsCategory-edit";
    import NewsCategoryDetail
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const newsCategoryGrid = ref({})// as VxeGridInstance
            // 搜索表单是否展开
            ...
            const newsCategoryListApp = reactive({
            ...
            const gridOptions = reactive({
                ...
                proxyConfig: {
                    ...
                    ajax: {
                        ...
                        query: ({page, sorts, filters}) => {  // 接收 Promise
                            ...
                            return NewsCategoryService.findNewsCategorys(queryParams).catch(error => {
                ...
                exportConfig: {
                    ...
                    exportMethod({options}) {
                        ...
                        // 开始服务端导出
                        return NewsCategoryService.findNewsCategorysForExport(body).then(data => {
            ...
            provide('newsCategoryListApp', newsCategoryListApp)
            provide('gridOptions', gridOptions)

            /*TODO:新建事件*/
            const addEvent = () => {
                    newsCategoryListApp.editModalShowing = true
                    newsCategoryListApp.currentId = null;
                    newsCategoryListApp.addModalForEdit = true;
                gridOptions.showEditTitle = "新建";
            }

            /*TODO:新建独立页*/
            const addEvent2 = () => {
                router.push(`/cms/newsCategory/newsCategory/newsCategory-add`);
            }
            /*TODO:编辑事件*/
            const editEvent = (row) => {
                    newsCategoryListApp.editModalShowing = true
                    newsCategoryListApp.currentId = row.eid;
                    newsCategoryListApp.editModalForEdit = true;
                gridOptions.showEditTitle = "编辑";
            }
            /*TODO:编辑独立页*/
            const editEvent2 = (eid) => {
                router.push(`/cms/newsCategory/newsCategory/newsCategory-edit/${eid}`);
            }
            ...
            const viewEvent = (row) => {
                    newsCategoryListApp.currentId = row.eid;
                    newsCategoryListApp.viewModalShowing = true
                gridOptions.showDetailsTitle = '查看';
            }

            /*TODO:删除事件*/
            const deleteEvent = (row) => {
                    NewsCategoryService.deleteNewsCategory(row.eid).then(() => {
